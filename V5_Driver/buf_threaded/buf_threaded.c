...
static ssize_t driver_write(struct file *instance,
...
void GenStackNew(genStack * s, size_t elemSize,
...
static void thread_read(struct work_struct *work)
{
	...
	data->read_data->user =
	    (char *) kmalloc(sizeof(max_line_size), GFP_KERNEL);
			    ...
			    loff_t * off)
{
	...
	data->write_data->user =
	    kmalloc(sizeof(max_line_size), GFP_KERNEL);
			   ...
			   loff_t * offset)
{
	...
	pr_debug
	    ("not_copied: %lu to_copy: %lu. count %d. %lu bytes read\n",
